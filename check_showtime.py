...
class JamorPlinicex :
    ...
    def check_showtime(self) :
        result = ""
        for i in self.__cineplex_list :
            result += f"\nJamor Plinicex {i.name}\n\n"
            movie = i.get_movie()
            for j in movie :
                result += f"\t{j.get_movie_name()}\n"
                showtime = j.get_showtime()
                theater = showtime[0].get_theater()
                result += f"\t\tTheater {theater.get_theater_id()}\n"
                for k in showtime :
                    result += f"\t\t\t{k.get_time()}\n"
        return result
...
class Showtime :
    def __init__(self,name,start_time,end_time,subtitle,price,theater):
        self.__movie_name = name
    ...
    def get_time(self) :
        result = f"{self.__start_time} - {self.__end_time}"
        return result
...
robinsonlad = testjamorplinicex.add_cineplex("Robinson Ladkrabang")
lotuslad = testjamorplinicex.add_cineplex("Lotus Ladkrabang")
cenlad = testjamorplinicex.add_cineplex("Central Ladprao")
...
def create_seats():
    ...
    for j in range(1,13):
        ...
        # Normal
        for i in ["A","B","C"]:
            seats.append(NormalSeat(f"{i}{j}"))

        # Sofa
        for i in ["D","E","F"]:
            seats.append(SofaSeat(f"{i}{j}"))

        # Honeymoon
        for i in ["G","H","I"]:
            seats.append(HoneyMoonBed(f"{i}{j}"))

    return seats


def create_showtime(movie, times, price, theater):
    showtimes = []
    for start,end in times:
        showtimes.append(
            Showtime(movie,start,end,"EN",price,theater)
        )
    return showtimes
...
def create_mock_data():
    ...
    bank = Bank("krungjean")
    bank.create_account("Potapo","1112",2500)

    # Cineplex
    robinsonlad = system.add_cineplex("Robinson Ladkrabang")
    lotuslad = system.add_cineplex("Lotus Ladkrabang")
    cenlad = system.add_cineplex("Central Ladprao")
    ...
    # Theater
    t1 = robinsonlad.add_standard_theater("01",s1)
    t2 = robinsonlad.add_imax_theater("02",s2)
    t3 = lotuslad.add_standard_theater("01",s3)
    t4 = lotuslad.add_imax_theater("03",s4)
    t5 = cenlad.add_standard_theater("01",s5)
    t6 = cenlad.add_4dx_theater("03",s6)
    ...
    # Showtime
    sh1 = create_showtime("Harry Potter",harry_times1,100,t1)
    sh2 = create_showtime("Harry Potter",harry_times2,100,t3)
    sh3 = create_showtime("Harry Potter",harry_times3,120,t5)

    sh4 = create_showtime("Doraemon The Movie",dora_times1,120,t2)
    sh5 = create_showtime("Doraemon The Movie",dora_times2,120,t4)
    sh6 = create_showtime("Doraemon The Movie",dora_times3,120,t6)

    # Movie
    robinsonlad.add_movie("Harry Potter",3,"Fantasy","13+",sh1)
    lotuslad.add_movie("Harry Potter",3,"Fantasy","13+",sh2)
    cenlad.add_movie("Harry Potter",3,"Fantasy","13+",sh3)

    robinsonlad.add_movie("Doraemon The Movie",2.5,"Cartoon","8+",sh4)
    lotuslad.add_movie("Doraemon The Movie",2.5,"Cartoon","8+",sh5)
    cenlad.add_movie("Doraemon The Movie",2.5,"Cartoon","8+",sh6)

    return system, bank

testjamorplinicex, krungjean = create_mock_data()
...
print(testjamorplinicex.check_showtime())