...
class ExchangeCoupon(Coupon):
    def __init__(self, coupon_id, name, goods_list):
        super().__init__(coupon_id, name, discount=0)
        self.__list_goods = goods_list
...
class Cineplex:
    ...
    def search_movie_by_id(self, movie_id):
        for m in self.__movies_list:
            if m.id == movie_id:
                return m
        return None

    def search_theater_by_id(self, theater_id):
        for t in self.__theaters_list:
            if t.id == theater_id:
                return t
        return None
...
class JamorCineplex:
    ...
    def process_create_showtime(self, cineplex_id, showtime_id, movie_id, theater_id,
        ...
        new_showtime = Showtime(showtime_id, movie, theater, start_time, end_time, base_price,
